...
class QuestionnaireManager (private val questionnaireDao: QuestionnaireDao) {
    ...
    fun getAllQuestionnaire(): Observable<List<Questionnaire>> {
        return questionnaireDao.getAll().subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread())
    }
    fun getQuestionsByQuestionnaireId(id:Long): Observable<List<QuestionnaireWithQuestions>> {
        return questionnaireDao.getQuestionnaireByIdWithQuestions(id).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread())
    }
}